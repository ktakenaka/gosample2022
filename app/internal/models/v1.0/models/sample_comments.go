...
type SampleComment struct {
	...
	SampleID  uint64      `boil:"sample_id" json:"sample_id" toml:"sample_id" yaml:"sample_id"`
...
var SampleCommentWhere = struct {
	...
	SampleID  whereHelperuint64
	Content   whereHelpernull_String
	CreatedAt whereHelpertime_Time
}{
	ID:        whereHelperuint64{field: "`sample_comments`.`id`"},
	SampleID:  whereHelperuint64{field: "`sample_comments`.`sample_id`"},
	Content:   whereHelpernull_String{field: "`sample_comments`.`content`"},
	CreatedAt: whereHelpertime_Time{field: "`sample_comments`.`created_at`"},
}
...
var (
	sampleCommentAllColumns            = []string{"id", "sample_id", "content", "created_at"}
	sampleCommentColumnsWithoutDefault = []string{"sample_id", "content"}
	sampleCommentColumnsWithDefault    = []string{"id", "created_at"}
	sampleCommentPrimaryKeyColumns     = []string{"id"}
)

type (
	// SampleCommentSlice is an alias for a slice of pointers to SampleComment.
	// This should generally be used opposed to []SampleComment.
...
func FindSampleComment(ctx context.Context, exec boil.ContextExecutor, iD uint64, selectCols ...string) (*SampleComment, error) {
...
func (o *SampleComment) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = uint64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == sampleCommentMapping["id"] {
		goto CacheNoHooks
	}
...
func SampleCommentExists(ctx context.Context, exec boil.ContextExecutor, iD uint64) (bool, error) {
	var exists bool
