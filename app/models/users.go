...
var UserRels = struct {
	OfficesUsers     string
	CreatedBySamples string
	UpdatedBySamples string
}{
	OfficesUsers:     "OfficesUsers",
	CreatedBySamples: "CreatedBySamples",
	UpdatedBySamples: "UpdatedBySamples",
}

// userR is where relationships are stored.
type userR struct {
	OfficesUsers     OfficesUserSlice `boil:"OfficesUsers" json:"OfficesUsers" toml:"OfficesUsers" yaml:"OfficesUsers"`
	CreatedBySamples SampleSlice      `boil:"CreatedBySamples" json:"CreatedBySamples" toml:"CreatedBySamples" yaml:"CreatedBySamples"`
	UpdatedBySamples SampleSlice      `boil:"UpdatedBySamples" json:"UpdatedBySamples" toml:"UpdatedBySamples" yaml:"UpdatedBySamples"`
}
...
func (o *User) OfficesUsers(mods ...qm.QueryMod) officesUserQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`offices_users`.`user_id`=?", o.ID),
	)
...
// CreatedBySamples retrieves all the sample's Samples with an executor via created_by column.
func (o *User) CreatedBySamples(mods ...qm.QueryMod) sampleQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`samples`.`created_by`=?", o.ID),
	)
...
// UpdatedBySamples retrieves all the sample's Samples with an executor via updated_by column.
func (o *User) UpdatedBySamples(mods ...qm.QueryMod) sampleQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`samples`.`updated_by`=?", o.ID),
	)
...
func (userL) LoadOfficesUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`offices_users.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadCreatedBySamples allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatedBySamples(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`samples`),
		qm.WhereIn(`samples.created_by in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatedBySamples = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &sampleR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.CreatedBy) {
				local.R.CreatedBySamples = append(local.R.CreatedBySamples, foreign)
				if foreign.R == nil {
					foreign.R = &sampleR{}
				}
...
// LoadUpdatedBySamples allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUpdatedBySamples(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`samples`),
		qm.WhereIn(`samples.updated_by in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UpdatedBySamples = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &sampleR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.UpdatedBy) {
				local.R.UpdatedBySamples = append(local.R.UpdatedBySamples, foreign)
				if foreign.R == nil {
					foreign.R = &sampleR{}
				}
...
func (o *User) AddOfficesUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*OfficesUser) error {
	var err error
...
// AddCreatedBySamples adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatedBySamples.
// Sets related.R.CreatedByUser appropriately.
func (o *User) AddCreatedBySamples(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Sample) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		o.R = &userR{
			CreatedBySamples: related,
		}
	} else {
		o.R.CreatedBySamples = append(o.R.CreatedBySamples, related...)
	}
...
// AddUpdatedBySamples adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.UpdatedBySamples.
// Sets related.R.UpdatedByUser appropriately.
func (o *User) AddUpdatedBySamples(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Sample) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		o.R = &userR{
			UpdatedBySamples: related,
		}
	} else {
		o.R.UpdatedBySamples = append(o.R.UpdatedBySamples, related...)
	}
